...
export interface StudyBuilderRequest {
  messages: AssistantMessage[];
  study_name?: string;
  existing_groups?: Record<string, string[]>;
  ratio_definitions?: RatioDefinition[];
}

export interface StudyBuilderResponse {
  reply: string;
  suggested_study_name?: string | null;
  suggested_groups: Record<string, string[]>;
  next_questions: string[];
}

export interface InterpretationGroupSummary {
  group: string;
  mean: number;
  sd?: number | null;
  count: number;
...
export interface InterpretationRequest {
  metric_id: string;
  metric_label: string;
  group_summaries: InterpretationGroupSummary[];
  reference_group?: string | null;
  significance_notes?: string | null;
  thresholds?: Record<string, number>;
}
